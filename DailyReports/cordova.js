// This is needed because Cordova 3.0.0 executes PluginManager service call before our exec implementation. 
// That causes the exec to fallback to prompt, which results in ugly message at the beggining.
window._cordovaNative = {
	exec: function (success, fail, service, action, args) {
		...
		ICE_CordovaExecutionProcessor.Execute(service, action, callbackId, JSON.stringify(args));
	}
};
...
if (window.navigator.__defineGetter__) {
	window.navigator.__defineGetter__('userAgent', function () {
		return 'Mozilla/5.0 (iPhone; U; CPU iPhone OS 6_0 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8F190 Safari/6533.18.5';
	});
}
...
// ICE_CordovaExecutionProcessor is used in simulator-utils-suffix file
if (typeof ICE_CordovaExecutionProcessor === "undefined") {
	ICE_CordovaExecutionProcessor = {};
}

if (typeof ICE_SimulatorBridge === "undefined") {
	ICE_SimulatorBridge = {};
}

(function (ICE_SimulatorBridge) {
	ICE_SimulatorBridge.utils = {
	...
	ICE_SimulatorBridge.updateOrietation = function (newOrientation) {
		window.orientation = newOrientation;
		var doc = window.document,
		evt = doc.createEvent("Events");
		evt.initEvent("orientationchange", true, true);
		if (typeof window.onorientationchange == 'function') {
			window.onorientationchange(evt);
		}
	...
	function _raiseTouchEvent(mouseEvent) {
		...
		switch (mouseEvent.type) {
			case "mousedown":
				type = "touchstart";
				_isMouseDown = true;
				_isRightAfterDown = true;
				break;
			case "mousemove":
				if (!_isMouseDown) {
					return;
				}
				if (_isRightAfterDown) {
					mouseEvent.preventDefault();
					_isRightAfterDown = false;
					return;
				}
				type = "touchmove";
				preventDefault = true;
		...
		var touchList = new TouchList(touchObj);
		...
		if (emptyTouches) {
			eventData.touches = new TouchList();
		}
		ICE_SimulatorBridge.utils.mixin(touchObj, eventData);

		simulatedEvent = _initTouchEvent(type, true, true, eventData);

		if (!_isInternalRaise) {
			_isInternalRaise = true;
			...
			_isInternalRaise = false;
		}
	}
	["mousedown", "mousemove", "mouseup"]
		.forEach(function (event) {
					document.addEventListener(event, _raiseTouchEvent, true);
		});

	var notSupportedInputTypes = ["datetime-local", "date", "month", "week"]
	var cancelNotSupportedElements = function (ev) {
		var target = ev.target;
		if (target && target.type &&
			notSupportedInputTypes.indexOf(target.type.toLowerCase()) >= 0) {
			ev.preventDefault();
		}
	};

	document.addEventListener("click", cancelNotSupportedElements, true);
});
...
cordova.define('cordova/exec', function (require, exports, module) {
	function simulatorExec(success, fail, service, action, args) {
		var callbackId = service + cordova.callbackId++;
	...
	simulatorExec.nativeCallback = function (callbackId, status, payload, keepCallback) {
		var success = status === 0 || status === 1;
		if (success) {
			cordova.callbackSuccess(callbackId, { status: status, message: payload, keepCallback: keepCallback });
		} else {
			cordova.callbackError(callbackId, { status: status, message: payload, keepCallback: keepCallback });
		}
	};

	module.exports = simulatorExec;
});
...
var platformModule = cordova.require('cordova/platform');

if(platformModule.clobbers)
{
	delete platformModule.clobbers.console;
}

if(platformModule.objects && platformModule.objects.console)
{
	delete platformModule.objects.console;
}

cordova.define.remove('cordova/plugin/ios/logger/symbols');